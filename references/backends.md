...
`bin/backends/*.sh`, re-verify it on a smoke task (see "Adding a new job").

| Backend | Engine | Default model | Effort | Output format | Session for `ask` |
|---|---|---|---|---|---|
| glm | `claude -p` → z.ai | `glm-5.3[1m]` (`GLM_MODEL`) | `--effort max` | stream-json | `--session-id` our uuid → `--resume` |
| kimi | `claude -p` → api.kimi.com/coding | `kimi-for-coding[1m]` = K2.8 (`KIMI_MODEL`) | `--effort max` (= K3 max) | stream-json | same as glm |
| opus | `claude -p`, Claude subscription | `opus` (`OPUS_MODEL`) | `--effort max` | stream-json | same as glm |
| grok | `grok --prompt-file` | `grok-4.7` (`GROK_MODEL`) | `--effort xhigh` (maximum) | streaming-messages-json = claude stream-json format | `-s` our uuid → `--resume` |
| codex | `codex exec` | `gpt-6-astra` (`CODEX_MODEL`) | `-c model_reasoning_effort="ultra"` | `--json` JSONL + `-o` last message | `thread_id` from the `thread.started` event → `codex exec resume` |
| kimi-cli | `kimi -p` (OAuth) | `kimi-code/kimi-for-coding` (`KIMI_CLI_MODEL`) | from config.toml (`effort = "high"|"max"`) | stream-json (lines without `type`) | `session_id` from `session.resume_hint` → `-S` |

Default models and effort live in `bin/lib/defaults.sh`; override with `quorum config set KEY VALUE`
(file `~/.config/quorum/config.env`) or an environment variable for a single run.

Isolation is the same for all: a detached git worktree at a temporary commit of the working tree
(uncommitted and untracked files included, .gitignore respected). The snapshot is writable — the job
...
`.env*`) are copied into the snapshot by default (`QUORUM_DEPS=copy`); `--deps hardlink|symlink|none`
switches the mode, and under `symlink` they point into the original tree, so the job is
...
- `--output-format stream-json --verbose`: progress is visible as it happens; the final text is in the
  ...
  is empty — take the last assistant text, then `ANSWER.md` from the snapshot.
- `quorum ask` for the claude family = `claude -p --resume <uuid>` in the same snapshot and environment: the job
  sees its own session, can run further checks and update `ANSWER.md` (verified on glm 2026-09-02, reply in ~1 min).
- glm and kimi run with a separate `CLAUDE_CONFIG_DIR` (`~/.local/state/quorum/cfg/<name>`):
  user settings, plugins and hooks are not loaded; job sessions are stored separately.
...
- z.ai: the `[1m]` suffix works only through Claude Code (`ANTHROPIC_MODEL`); on the raw API,
  ...
  `API Error: Connection closed mid-response` happen — restart the job. In stderr,
...
When the job must not get access to host services, the claude family runs in a container
...
(`mkdir -p "$SNAP/vendor"`). In this mode the job cannot run tests.
...
Then a smoke task (`quorum consult --prompt-file smoke.md`), then a run on a fixture with
known defects — that shows whether the job hallucinates.
