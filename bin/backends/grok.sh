...
case "$1" in
  run)
    sid="$(cat "$QR_DIR/session")"
    exec "$GROK_BIN" --prompt-file "$QR_PROMPT" --cwd "$QR_SNAPSHOT" \
  ...
  ask)
    ...
    sid="$(cat "$src/session")"
    exec "$GROK_BIN" -p "$2" --resume "$sid" --cwd "$QR_SNAPSHOT" \
      --sandbox "$(grok_sandbox)" --permission-mode bypassPermissions --output-format streaming-messages-json ;;
  format) echo claude-stream-json ;;   # grok streaming-messages-json = claude stream-json format
  check)
